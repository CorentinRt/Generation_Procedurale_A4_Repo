...
@onready var player_json: JSON = load("res://resources/dialogs/d_player.json")
...
func get_text_answer_from_type(answerType : RightAnswerType) -> String:
	...
	match answerType:
		RightAnswerType.PLAYER_NAME:
			answer = get_variable_text_from_json(player_json, "savedPlayerName")
		RightAnswerType.PLAYER_AGE:
			answer = get_variable_text_from_json(player_json, "savedPlayerAge")
